...
impl Simulation {
    ...
    pub async fn readback_cells(&self, device: &Device, queue: &Queue) -> Result<(), BufferAsyncError> {
        let size = size_of::<Cell>() as u64 * (self.width * self.height * self.depth) as u64;
        ...
        let mut encoder =
            device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        encoder.copy_buffer_to_buffer(&self.cells_buffer, 0, &staging_buffer, 0, size);
        queue.submit(Some(encoder.finish()));
        ...
        // TODO: return result and let consumer .. consume it :)
    ...
    pub async fn readback_objects(&self, device: &Device, queue: &Queue) -> Result<(), BufferAsyncError> {
        let size = size_of::<Object>() as u64 * (self.width * self.height * self.depth) as u64;
        let staging_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            size,
            label: Some("Simulation::staging_buffer"),
            usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
        ...
        let mut encoder =
            device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        encoder.copy_buffer_to_buffer(&self.objects_buffer, 0, &staging_buffer, 0, size);
        queue.submit(Some(encoder.finish()));
        ...
        let data = buffer_slice.get_mapped_range();
        let result = bytemuck::cast_slice::<u8, Object>(&data).to_vec();
        staging_buffer.destroy();

        // TODO: return result and let consumer .. consume it :)
        for (i, object) in result.iter().enumerate() {
    ...
    pub async fn readback_objects_length(&self, device: &Device, queue: &Queue) -> Result<(), BufferAsyncError> {
        let size = size_of::<u32>() as u64;
        let staging_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            size,
            label: Some("Simulation::staging_buffer"),
            usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
        ...
        let mut encoder =
            device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        encoder.copy_buffer_to_buffer(&self.objects_length_buffer, 0, &staging_buffer, 0, size);
        queue.submit(Some(encoder.finish()));
        ...
        // TODO: return result and let consumer .. consume it :)
        for (i, length) in result.iter().enumerate() {
